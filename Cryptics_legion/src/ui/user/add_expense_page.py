...
def create_add_expense_view(page: ft.Page, state: dict, toast, go_back,
    ...
    def show_view():
        ...
        # Helper function to format amount with thousand separators and decimals
        ...
        amount_field = ft.TextField(
            ...
            on_change=lambda e: (
                amount_value.update({'value': e.control.value.replace(',', '')}),
                setattr(e.control, 'value', format_amount(e.control.value)),
                page.update() if page else None
            ),
        )
...
def build_add_expense_content(page: ft.Page, state: dict, toast, go_back,
    ...
    # Add on_blur handler for amount field formatting and validation
    def on_amount_blur(e):
        """Format amount input, validate against balance when user leaves field."""
        formatted = format_amount(e.control.value)
        ...
        if formatted:
            ...
            try:
                amount = float(formatted.replace(",", ""))
                current_acc = get_account_info(expense_state["selected_account_id"])
                
                if current_acc:
                    available_balance = current_acc[4]  # acc[4] is the balance
                    acc_currency = current_acc[5] if len(current_acc) > 5 else "PHP"
                    acc_symbol = get_currency_symbol(acc_currency)
                    acc_name = current_acc[1]
                    
                    if amount > available_balance:
                        amount_error.value = f"⚠️ Exceeds {acc_name} balance ({acc_symbol}{available_balance:,.2f})"
