...
def create_home_view(page: ft.Page, state: dict, toast, show_dashboard, logout_callback,
    ...
    def show_all_expenses(e):
        ...
        from ui.user.all_expenses_page import create_all_expenses_view
...
def build_home_content(page: ft.Page, state: dict, toast, 
    ...
    # Extract first name - use actual first_name if filled, otherwise use first part of full_name
    if user_profile:
        first_name = user_profile.get("first_name", "").strip()
        if not first_name and user_profile.get("full_name"):
    ...
    last_name = user_profile.get("last_name", "").strip() if user_profile else ""
