...
def create_personal_details_view(page: ft.Page, state: dict, toast, on_complete, on_back=None):
    ...
    def show_view():
        ...
        def handle_continue(e):
            ...
            # Split full_name into first_name and last_name if not already set
            if form_data["full_name"].strip() and (not form_data["first_name"] or not form_data["last_name"]):
                full_name_parts = form_data["full_name"].strip().split(" ", 1)
                form_data["first_name"] = full_name_parts[0]
                form_data["last_name"] = full_name_parts[1] if len(full_name_parts) > 1 else full_name_parts[0]
