...
from utils.biometric import authenticate_biometric, is_biometric_available
import hashlib
import threading
...
def create_passcode_setup(page: ft.Page, state: dict, on_complete):
    ...
    def on_biometric():
        """Handle biometric button - show info during setup."""
        info_dialog = ft.AlertDialog(
            title=ft.Row([
                ft.Icon(ft.Icons.FINGERPRINT, size=32, color=theme.accent_primary),
                ft.Text("Biometric Security", size=16, weight=ft.FontWeight.BOLD),
            ], spacing=12),
            content=ft.Text(
                "After setup, you can use biometric authentication (fingerprint, face, or device biometric) to unlock the app.",
                size=14,
                color=theme.text_secondary,
            ),
            ...
            actions=[
                ft.TextButton(
                    "OK",
                    on_click=lambda e: page.close(info_dialog),
...
def create_passcode_verify(page: ft.Page, state: dict, on_success, on_forgot=None):
    ...
    first_name = user_profile.get("first_name", "User") if user_profile else "User"
    if not first_name and user_profile and user_profile.get("full_name"):
        # Fallback: extract first word from full_name
        first_name = user_profile.get("full_name", "").split()[0] if user_profile.get("full_name") else "User"
    ...
    def on_biometric():
        ...
        if not is_biometric_available():
            error_text.value = "⚠️ Biometric not available on this device"
            error_text.visible = True
            page.update()
            return
        ...
        try:
            ...
            biometric_dialog = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton(
                        "Authenticate",
                        on_click=lambda e: authenticate_biometric_action(biometric_dialog),
                    ),
                    ft.TextButton(
                        "Use Passcode",
    ...
    def authenticate_biometric_action(dialog):
        """Call the cross-platform biometric authentication."""
        dialog.open = False
        page.update()
        
        def on_biometric_success():
            """Called when biometric authentication succeeds."""
            db.set_biometric_enabled(state["user_id"], True)
            on_success()
        
        def on_biometric_error(error_msg: str):
            """Called when biometric authentication fails."""
            error_text.value = f"⚠️ {error_msg}"
            error_text.visible = True
            page.update()
        
        # Trigger cross-platform biometric
        authenticate_biometric(first_name, on_biometric_success, on_biometric_error)
